...
uint32_t maxIndex1;  /* Index in Output array where max value is */
uint32_t maxIndex2;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim == &htim2)
  {
    ...
    count++;
    HAL_GPIO_TogglePin(GPIOG, ADC_TOG_Pin);
  }
}
...
//  arm_cfft_radix4_instance_f32  FFT_F32_struct;
  ...
  /* Convert the dataset into complex form */
...
//  }
  ...
  for (uint16_t i = fftSize / 2 + 1; i <= fftSize; i++)
  {
	fftOutput[i] = 0;
  }
...
//  arm_max_f32(fftOutput_half, fftSize / 2 + 1, &maxValue, &maxIndex);
  arm_max_f32(fftOutput, fftSize, &maxValue, &maxIndex1);

  fftOutput[maxIndex1] = 0;
  arm_max_f32(fftOutput, fftSize, &maxValue, &maxIndex2);
  /* choose the fundamental frequency */
  if (maxIndex1 <= maxIndex2)
  {
	/* one bin on the spectrum = 16k / 4096 */
	frequency = maxIndex1 * 16000 / fftSize;
  }
  else
  {
	frequency = maxIndex2 * 16000 / fftSize;
  }
...
//  printf("%f\n\r", fftOutput[i]);
  ...
  if (frequency >= 250)
  {
    frequency = frequency * 0.96; /* remove the error */
  }
