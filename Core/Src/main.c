...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ADC_CONVERTED_DATA_BUFFER_SIZE  ((uint32_t)  1024)
...
float32_t fftInput[FFT_Length_Tab * 2];
static float32_t fftOutput[FFT_Length_Tab];

uint8_t ADC_covert_complete_flag;
...
int main(void)
{
  ...
  MX_TIM2_Init();
  /* USER CODE BEGIN 2 */
  HAL_ADC_Start_DMA(&hadc3, (uint32_t*) &aADCxConvertedData, ADC_CONVERTED_DATA_BUFFER_SIZE);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    if(ADC_covert_complete_flag)
    {
      ADC_covert_complete_flag = 0;
//      printf("ADC_Convert data:\n\r");
//      for(i=0;i<ADC_CONVERTED_DATA_BUFFER_SIZE;i++)
//	    printf("%d ",aADCxConvertedData[i]);
//      printf("\n\r");
...
/* USER CODE BEGIN 4 */
//void HAL_TIM_PeirodElapseCallback(TIM_HandleTypeDef *htim)
//{
...
//  arm_cfft_radix4_instance_f32  FFT_F32_struct;
  ...
  for (index_fill_input_buffer = 0; index_fill_input_buffer < fftSize * 2; index_fill_input_buffer += 2)
  {
    /*The data in the complex arrays is stored in an interleaved fashion
      (real, imag, real, imag, ...).*/
    fftInput[(uint16_t)index_fill_input_buffer] = (float32_t) aADCxConvertedData[index_fill_input_buffer / 2] / (float32_t) 4096.0 * 3.3;
    /* Imaginary part */
...
//  printf("FFT(F32) maxValue %f, maxIndex %ld\n\r", maxValue, maxIndex);
